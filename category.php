        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
					...
					$cat_name = mysqli_query($conn, "SELECT * FROM category WHERE category_id={$cat_id}");
					while($cat_name_fetch = mysqli_fetch_assoc($cat_name)){
						echo "<h2 class='page-heading'>{$cat_name_fetch['category_name']}</h2>";
					}
					?>
					...
					$sql = mysqli_query($conn, "SELECT * FROM `post` JOIN category ON `post`.category = `category`.category_id
						       JOIN `user` ON `post`.author = `user`.user_id WHERE category.category_id={$cat_id} ORDER BY post.post_id DESC LIMIT {$offset},{$limit}");
					...
					?>
					<?php
					   $pagination = mysqli_query($conn, "SELECT post FROM `category` WHERE category_id={$cat_id}");
					  
					   if(mysqli_num_rows($pagination) > 0){
						   $fetch = mysqli_fetch_assoc($pagination);
						   
						   $total_records = $fetch['post'];
