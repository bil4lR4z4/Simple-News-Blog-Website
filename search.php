            ...
            <div class="col-md-8">
				...
				<?php
					...
					$sql = mysqli_query($conn, "SELECT * FROM `post` JOIN category ON `post`.category = `category`.category_id
							   ...
							   WHERE post.title LIKE '%{$search}%' OR post.description LIKE '%{$search}%' OR category.category_name LIKE '%{$search}%' OR user.username LIKE '%{$search}%'
                   ...
                   <?php 
					...
					?>
					<?php
					   $pagination = mysqli_query($conn, "SELECT * FROM `post` WHERE post.title='{$search}' OR post.description='{$search}'");
